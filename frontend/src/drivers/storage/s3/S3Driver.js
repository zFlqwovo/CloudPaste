...
export class S3Driver {
  ...
  applyFsUploader(uppy, { strategy, path } = {}) {
    ...
    if (strategy === STORAGE_STRATEGIES.S3_BACKEND_DIRECT) {
      ...
      const headers = {};
      if (auth.authType === "admin" && auth.adminToken) headers["Authorization"] = `Bearer ${auth.adminToken}`;
      else if (auth.authType === "apikey" && auth.apiKey) headers["Authorization"] = `ApiKey ${auth.apiKey}`;
